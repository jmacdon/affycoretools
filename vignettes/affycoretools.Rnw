...
\textbf{Please note that most of the following is now outdated. This
  document will remain for one more release cycle while all annaffy
  based tools are deprecated, and once they become defunct this
  document will be removed. Please see the replacement vignette called
  UsingReportingTools, which explains how to use the refactored
  functions that now rely on ReportingTools for HTML output.}
...
\cite{ESS} and an \Rfunction{Sweave} document \cite{Leisch:2002}.
