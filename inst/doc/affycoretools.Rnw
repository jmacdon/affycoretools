...
library(affycoretools)
dat <- ReadAffy(celfile.path = "../examples")
eset <- rma(dat)
@ 
...
plotHist(dat, list.celfiles("../examples"))
@ 
...
\centering
<<fig=true, width=6, height=6, echo = false>>=
plotDeg(dat, list.celfiles("../examples"))
@ 
...
\centering
<<fig=true, width=6, height=6, echo = false>>=
pd <- read.phenoData("../examples/pdata.txt", header = TRUE, row.names = 1)
...
vc <- vennCounts2(fit2, rslt, method = "same")
