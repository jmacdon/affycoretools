...
\SweaveOpts{ prefix.string= ../examples/affycoretools}
...
For these examples, I will be using some data that was generated in our microarray
...
<<>>=
setwd(system.file("examples", package = "affycoretools"))
options(width = 70)
@ 

The preceding code simply points R to the correct working directory and sets the 
page width in R so the output fits on this page.
...
<<>>=
eset1 <- affystart(filenames = list.celfiles()[1:6], plot = FALSE, pca = FALSE)
eset2 <- affystart(filenames = list.celfiles()[7:12], plot = FALSE, pca = FALSE)
...
contrasts <- cbind(c(1, -1, 0, 0), c(0, 0, 1, -1))
ugrps <- unique(grps)
dimnames(contrasts) <-  list(ugrps, c(paste(ugrps[1], ugrps[2], sep = " - "),
                      paste(ugrps[3],ugrps[4], sep = " - ")))
...
<<eval = false>>=
vennSelect(eset, fit2, design, rslt)
@ 
...
rate). The gene lists will be sorted in descending $p$-value order, so theoretically
...
probids <- geneNames(eset)[index1]
hyperGtable(probids, annotation(eset), filename = "MF GO terms")
@ 

This function outputs a text file that can be opened in e.g., a spreadsheet
program.
