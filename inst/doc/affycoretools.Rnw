...
pd <- new("AnnotatedDataFrame", data = read.table("pdata.txt", header = TRUE, row.names = 1))
...
pd <- new("AnnotatedDataFrame", 
          data = read.table("../examples/pdata.txt", header = TRUE, row.names = 1))
...
eset <- new("ExpressionSet", exprs = cbind(exprs(eset1), exprs(eset2)), phenoData = pd,
